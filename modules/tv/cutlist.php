...
$marks = $db->query_list_assoc('SELECT mark,type,data FROM recordedmarkup WHERE chanid=? AND starttime=FROM_UNIXTIME(?) AND type IN (0,1,32,34) ORDER BY mark', $program->chanid, $program->starttime);
...
foreach($marks as $mark) {
    ...
    case 1:
        $cuts[] = [
		'frame' => intval($mark['mark']),
		'type' => intval($mark['type'])
	];
        break;
    case 32:
    	$framerate = [
		'frame' => intval($mark['mark']),
		'rate' => intval($mark['data'])
	];
	$framerate['ms'] = $last_framerate['ms'] + ($framerate['frame'] - $last_framerate['frame']) / $last_framerate['rate'];
    	$framerates[] = $last_framerate = $framerate;
	break;
    case 34:
        $frames = intval($mark['data']);
	break;
    }
}
...
if (in_array($program->hostname, $online_backends)) {
	$preview_url = 'http://'._or(setting('BackendServerIP', $program->hostname), $program->hostname).':'._or(setting('BackendStatusPort', $program->hostname), _or(setting('BackendStatusPort'), 6544)).'/Content/GetPreviewImage?RecordedId='.$program->recordedid.'&FramesIn=';
} else {
	$preview_url = skin_img_url.'tv.png?frame=';
}

// Display the page
    require_once tmpl_dir.'cutlist.php';
